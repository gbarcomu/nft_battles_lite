...
import { fetchQuestStage, playQuest, getRemainingTime } from '../ethereumConnector';
    ...
    try {
      fetchQuestStage().then(data => {
        if (parseInt(data) === 1) {
          ...
          setQuestStage(<p>There isn't any quest outstanding, wait until there is one or create it!</p>);
        }
        else {
          getRemainingTime().then(remTime => {
            setQuestStage(<div><p>There is an ongoing quest, wait unit it is resolved.</p>
            <p>In {remTime.toString()} seconds, you can force the termination of the quest for inactivity</p></div>);
          })
        }
      });
    }
